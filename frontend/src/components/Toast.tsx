...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ToastContext.Provider value={{ show }}>
      {children}
      <div className="workbench-toast-stack fixed bottom-4 right-4 z-50 flex flex-col gap-2">
        {toasts.map((t) => (
          <div
            key={t.id}
            className={`workbench-toast workbench-toast--${t.type} flex items-center gap-2 rounded-lg px-4 py-3 shadow-lg ${
              t.type === 'success'

...
async def upload_template(
    ...
    """上传 Excel 模板。"""
    template = template_service.upload_template(db, file)
    return template_service.template_to_info(template)


@router.get("/current")
async def get_current_template(db: Session = Depends(get_db)) -> dict[str, Any] | None:
    """获取当前活跃模板配置。"""
    template = template_service.get_active_template(db)
    if template is None:
        return None
    return template_service.template_to_info(template)
...
async def get_sheets(
    ...
    """读取模板所有工作表。"""
    template = template_service.get_template_or_404(db, template_id)
